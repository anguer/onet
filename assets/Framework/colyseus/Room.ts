...
import { SocketMessageEvent } from 'db://assets/Framework/factories/socket/Socket';
...
export class Room<State = any> {
  ...
  constructor(name: string, rootSchema?: SchemaConstructor<State>) {
    ...
    this.onError((code, message) => LogManager.warn('[colyseus.js]', `onError => (${code}) ${message}`));
  ...
  public connect(
    ...
    connection.events.onclose = function (e) {
      if (!room.hasJoined) {
        LogManager.warn('[colyseus.js]', `Room connection was closed unexpectedly (${e.code}): ${e.reason}`);
    ...
    connection.events.onerror = function (e) {
      LogManager.warn('[colyseus.js]', `Room, onError (${e.code}): ${e.reason}`);
      room.onError.invoke(e.code, e.reason);
    };
  ...
  public onMessage<T = any>(type: '*', callback: (type: string | number, message: T) => void): () => void;
  public onMessage<T = any>(type: string | number, callback: (message: T) => void): () => void;
  public onMessage(type: '*' | string | number, callback: (...args: any[]) => void): () => void {
    return this.onMessageHandlers.on(this.getMessageHandlerKey(type), callback);
  }

  public send<T = any>(type: string | number, message?: T): void {
    const it: schema.Iterator = { offset: 1 };
  ...
  protected onMessageCallback(event: SocketMessageEvent) {
    if (typeof event.data === 'string') {
      LogManager.warn('[colyseus.js]', 'The Message does not support string type.');
      return;
    }
    ...
    if (code === Protocol.JOIN_ROOM) {
      ...
      // acknowledge successfully JOIN_ROOM
      ...
      const message = buffer.byteLength > it.offset ? unpack(buffer, { start: it.offset }) : undefined;
  ...
  private dispatchMessage(type: string | number, message: any) {
    ...
    if (this.onMessageHandlers.events[messageType]) {
      ...
      LogManager.warn('[colyseus.js]', `onMessage() not registered for type '${type}'.`);
    }
  }
